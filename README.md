...
colours 0.02 apart, the same two stops blended in oklab and in the lamps.
`./bin/visualdocs ramp --css` says a page in css; `--go` prints the
source that painted it, so picture, sheet and code cannot drift apart.

one person keeps a theme in a terminal, an editor, a web page and a
lamp, and matches them by eye, one hex code at a time. this library
gives them one primitive, the swatch, and works up from it, so the same
colour is the same colour everywhere it lands.
