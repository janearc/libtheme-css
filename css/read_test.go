...
.scanlines { background: repeating-linear-gradient(180deg, rgba(255,110,199,.09) 0 2px, transparent 2px 5px); }
.sun { background: radial-gradient(38% 26% at 50% 62%, #ff2d95 0%, #ff6ec7 30%, rgba(255,110,199,0) 72%); }
.sunset { background: linear-gradient(180deg, #2a0f4d 0%, #4a1361 34%, #8c2472 56%, #d94a8c 66%, #2b1250 80%, #160d2b 100%); }
:root {
  --sunset-1: #2a0f4d;
  --sunset-2: #4a1361;
  --sunset-3: #8c2472;
...
func TestReadRoles(t *testing.T) {
	...
	if len(names) != 11 || names[0] != "bg" || names[1] != "accent" || names[2] != "ink" {
		t.Fatalf("roles: %v", names)
	}
	if ink, _ := cw.Roles.Get("ink"); ink != srgb.MustHex("#ffffff").Swatch() {
...
func TestReadFamilyAlone(t *testing.T) {
	cw := Read(":root { --dusk-1: #000000; --dusk-2: #808080; --dusk-3: #ffffff; }")
	d := cw.Ramps["dusk"]
